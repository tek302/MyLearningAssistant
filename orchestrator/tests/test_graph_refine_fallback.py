"""Integration test for refine loop: pre low -> refine -> post still low -> fallback (Part 6C)."""
...
def mock_judge_low_scores():
    """Mock judge LLM response with low scores (both pre and post)."""
    ...
    mock_response.choices[0].message.content = json.dumps({
        ...
        "overall": 0.50,  # Below 0.75 threshold
        "reasons": ["Low overall score", "Needs improvement"]
    })
...
def test_refine_loop_fallback_path(mock_judge_low_scores, mock_synthesis_response, fixed_chunks):
    """Test integration: pre low -> refine -> post still low -> fallback (max one refine)."""
    # Capture events
    captured_events = []
    ...
    # Mock OpenAI client - always return low scores
    def mock_create(*args, **kwargs):
        messages = kwargs.get('messages', [])
        content = str(messages[-1].get('content', '')) if messages else ''
        
        # Judge call detection - always return low scores
    ...
    def stub_synthesize_answer(state):
        return {
            **state,
            "answer": "LangGraph is a library for building agent workflows [1].",
            "model": "gpt-4o-mini",
            "cannot_answer": False
        }
    ...
    with patch('app.graphs.rag_graph._get_openai_client', return_value=mock_client), \
        ...
        is_cannot_answer = (
            "I cannot answer this question based on the provided context." in answer_text or
            "I couldn't find any relevant information" in answer_text or
            "cannot answer" in answer_text.lower()
        )
        
        # Either fallback_used flag or cannot-answer message indicates fallback path
        assert fallback_used or is_cannot_answer, \
            f"Expected fallback behavior. fallback_used={fallback_used}, answer={answer_text[:200]}"
        
        # Citations should be empty when fallback is used
        if fallback_used or is_cannot_answer:
            citations = final_state.get("citations", [])
            assert len(citations) == 0, \
                f"Citations should be empty when fallback is used. Got {len(citations)} citations"
        
        # Assert events: judge(pre), policy(refine), refine, judge(post), policy(refine or fallback)
        ...
        # Should have 2 policy events (both should be refine or second is fallback)
        ...
        # Second policy should be refine (but graph routes to fallback due to max one refine)
        assert policy_events[1]["data"]["action"] in ["refine", "fallback"], \
            f"Second policy should be refine or fallback, got {policy_events[1]['data']['action']}"
        
        # Should have EXACTLY ONE refine event (max one refine iteration)
        assert len(refine_events) == 1, \
            f"Expected exactly 1 refine event (max one refine), got {len(refine_events)}: {refine_events}"
        
        # Verify refine_used is True
        assert final_state.get("refine_used", False), "refine_used should be True"
        
        # Verify that we did NOT refine twice (refine_info should not indicate a second refine)
        # The graph should route to fallback after the second policy returns refine
