"""Test PDF URL ingest endpoint."""

import pytest


def test_ingest_pdf_url(client, db_connection, cleanup_after_test, test_user_id):
    """Test ingesting a PDF URL (arXiv paper)."""
    ...
    assert "summary_id" in data, f"Response missing 'summary_id': {data}"
    ...
    assert data["embedding_count"] == data["chunk_count"], \
        f"Expected embedding_count == chunk_count, got {data['embedding_count']} != {data['chunk_count']}"
    assert data["content_type"] == "pdf", f"Expected content_type='pdf', got '{data['content_type']}'"
    assert data["summary_id"] != "", f"Expected non-empty summary_id, got '{data['summary_id']}'"
    ...
    with db_connection.cursor() as cur:
        ...
        cur.execute(
            "SELECT COUNT(*) FROM summaries WHERE source_id = %s AND kind = 'S1'",
            (source_id,)
        )
        summaries_count = cur.fetchone()[0]
        assert summaries_count >= 1, \
            f"Expected at least 1 S1 summary, got {summaries_count}"
